...
class DeliveryNote(db.Model):
    ...
    def repr(self, columns=None):
        order =Order.query.filter_by(fk_company_id=UserForCompany.query.filter_by(fk_user_id=current_user.id).first().fk_company_id).filter_by(id=self.fk_order_id).first()
        if not order:
            abort(404)
        ...
        _dict={
            ...
            'is_validated' : ('Annulé',"#CD212A") if self.is_validated is not None and self.is_validated == False else ('Livré','#00A170') if self.is_validated is not None and self.is_validated == True else ('Crée', '#34568B') ,
...
class Invoice(db.Model):
    ...
    def repr(self, columns=None):
        ...
        _dict={
            ...
            'order_category':Order.query.get(self.fk_order_id).category if self.fk_order_id else 'aucune categorie',
...
class Item(db.Model):
    ...
    fk_company_id = db.Column(db.Integer, db.ForeignKey('company.id'))
    fk_format_id = db.Column(db.Integer, db.ForeignKey('format.id'))
    fk_familly_id = db.Column(db.Integer, db.ForeignKey('format.id'))
    fk_aspect_id = db.Column(db.Integer, db.ForeignKey('aspect.id'))
    ...
    def repr(self, columns=None):
        _dict={
            ...
            'intern_reference' : self.intern_reference,
            'label' : self.label,
            'used_for': Utilisation.query.get(self.fk_familly_id).label if self.fk_familly_id and Utilisation.query.get(self.fk_familly_id) else '',
...
class Order(db.Model):
    ...
    def repr(self, columns=None):
        ...
        _dict={
            ...
            'total':'{:,.2f} DZD'.format(self.total),
            'quotation': Quotation.query.get(self.fk_quotation_id).intern_reference if self.fk_quotation_id else '/',
            'is_delivered' : ('pas reçus',"#d33723") if self.is_delivered is not None and self.is_delivered == False else ('livrée','#007256') if self.is_delivered is not None and self.is_delivered == True else None ,
