...
def admin_before_request():
    ...
    if current_user.is_authenticated:
        ...
        session['company_name'] = Company.query.get(user.fk_company_id).name
        if not user:
...
from root.models import Utilisation
@admin_bp.get('/products/families')
@admin_bp.post('/products/families')
@login_required
def families():
    ...
    _formats = Utilisation.query.filter_by(fk_company_id=c_id).all()
    liste = None
    if _formats:
        liste = [
            {
    ...
    if form.validate_on_submit():
        _format = Utilisation()
        f = Utilisation.query.filter_by(fk_company_id=c_id).filter(
            func.lower(Utilisation.label) == str.lower(form.label.data)).first()
        if f:
            _format = f
        ...
        return redirect(url_for('admin_bp.families'))

    return render_template('admin/families.html', form=form, liste=liste)


@admin_bp.get('/products/familles/<int:f_id>/edit')
@admin_bp.post('/products/familles/<int:f_id>/edit')
@login_required
def edit_familly(f_id):
    session['endpoint'] = 'product'
    form = FamillyForm()
    company = UserForCompany.query.filter_by(role="manager").filter_by(fk_user_id=current_user.id).first().fk_company_id
    format_ = Utilisation.query.filter_by(fk_company_id=company).filter_by(id=f_id).first()
    if not format_:
        return render_template("errors/404.html", blueprint="admin_bp")
    if request.method == "GET":
    ...
    if form.validate_on_submit():
        ...
        return redirect(url_for("admin_bp.families"))
    return render_template("admin/edit_familly.html", form=form)


@admin_bp.get('/products/familles/<int:f_id>/delete')
@login_required
def delete_familly(f_id):
    session['endpoint'] = 'product'
    _format = Format.query.get(f_id)
    if not _format:
        return render_template("errors/404.html", blueprint="admin_bp")
...
def add_product():
    ...
    __aspect = Aspect.query.filter_by(
        fk_company_id=UserForCompany.query.filter_by(role='manager').filter_by(fk_user_id=current_user.id) \
            .first().fk_company_id).all()
    _uses = Utilisation.query.filter_by(
        fk_company_id=UserForCompany.query.filter_by(role='manager').filter_by(fk_user_id=current_user.id) \
            .first().fk_company_id).all()
    if not _uses:
        flash('Il faut ajouter les familles des articles', 'warning')
    if not __formats:
    ...
    if form.validate_on_submit():
        ...
        item.fk_familly_id = form.utilisation.data.id if form.utilisation.data else None
...
def edit_product(item_id):
    ...
    if request.method == "GET":
        form = EditItemForm(
            ...
            utilisation=Utilisation.query.get(item.fk_familly_id) if item.fk_familly_id else None,
...
def add_quotation():
    ...
    _items = Item.query.filter_by(fk_company_id=company).all()
    for item in _items:
    ...
    if form.validate_on_submit():
        ...
        if enumerate(form.entities):
            for _index, entry in enumerate(form.entities):
                ...
                if entry.delete_entry.data:
                    sum_amounts -= entry.amount.data
                    del form.entities.entries[_index]
                    return render_template("admin/new_quotation.html",
                                           form = form, nested=EntryField(),
                                           somme = sum_amounts)
                _ = Entry()
                _.fk_item_id = entry.item.data.id
        ...
        if form.add.data:
            ...
            return render_template('admin/new_quotation.html', form=form, nested=EntryField(), somme=sum_amounts)
        if form.fin.data:
            if enumerate(form.entities):
                for _index, entry in enumerate(form.entities):
                    if entry.quantity.data:
        ...
        _q.total = sum_amounts
        _q.intern_reference = "DEV-1/" + str(datetime.now().date().year)
        if last_q:
            last_intern_ref = int(last_q.intern_reference.split('-')[1].split('/')[0])
            year = int(last_q.intern_reference.split('-')[1].split('/')[1])
            if year == datetime.now().date().year :
                _q.intern_reference = "DEV-"+str(last_intern_ref+1)+"/"+str(datetime.now().date().year)
        db.session.add(_q)
        db.session.commit()
        for e in entities:
            sum_amounts += e.total_price
        ...
        return render_template("admin/new_quotation.html", form=form,
                               somme = _q.total,
                               nested=EntryField(),
                               doc=_q.id,
                               to_save=False,
                               to_approve=True,
                               to_print=True)
    return render_template("admin/new_quotation.html", form = form, nested=EntryField(), somme = 0)



@admin_bp.get('/quotations/<int:q_id>/print')
@login_required
...
def orders():
    ...
    _orders = Order.query.filter_by(fk_company_id=user_for_company.fk_company_id) \
        .filter_by(category="vente").order_by(Order.created_at.desc()).all()
    liste = list()
    if _orders:
        indexe = 1
...
def add_client_order():
    ...
    if form.validate_on_submit():
        ...
        if enumerate(form.entities):
            for _index, entry in enumerate(form.entities):
                ...
                if not stock:
                    ...
                    return render_template("admin/new_client_order.html",
                                           form=form, nested=EntryField(),
                                           somme=sum_amounts)
                entry.unit.data = Item.query.get(entry.item.data.id).unit
                if entry.quantity.data:
        ...
        if form.add.data:
            ...
            return render_template('admin/new_client_order.html', form=form, nested=EntryField(), somme=sum_amounts)
        if form.fin.data:
            if enumerate(form.entities):
                for _index, entry in enumerate(form.entities):
                    if entry.quantity.data:
        ...
        return render_template("admin/new_client_order.html", form=form,
                               somme=_q.total,
                               nested=EntryField(),
                               to_approve=True,
                               doc=_q.id,
                               to_print=True)
    return render_template("admin/new_client_order.html", form=form, nested=EntryField(), somme=0)


from root.models import ExitVoucher
@admin_bp.get('/order/<int:o_id>/delete')
@login_required
def delete_order(o_id):
    company = UserForCompany.query.filter_by(role="manager") \
        .filter_by(fk_user_id = current_user.id).first()
...
def order_delivery(o_id):
    ...
    dl = DeliveryNote()
    previous_dl = DeliveryNote.query.filter_by(fk_order_id = cmd.id).first()
    if previous_dl:
        dl = previous_dl
    else:
        ...
        dl.intern_reference = "BL-1/"+str(datetime.now().year)
        if last_d:
            last_reference = int(last_d.intern_reference.split('-')[1].split('/')[0])
            year = int(last_d.intern_reference.split('-')[1].split('/')[1])
            if year == datetime.now().year:
                dl.intern_reference = "BL-"+str(last_reference+1)+"/"+ str(datetime.now().year)
    ...
    return redirect(url_for('admin_bp.print_delivery_note', bl_id=dl.id))



@admin_bp.get('/order/<int:o_id>/approve')
@login_required
...
@admin_bp.get('/quotations/<int:o_id>/get/invoice')
@login_required
...
@admin_bp.get('/order/<int:o_id>/print')
@login_required
...
def get_purchase_price():
    ...
    if data['cmd_id'] == '__None':
        return jsonify(
            text="Demande annulé: vous devez fournir le code de la commande"
        ), 400
...
def print_purchase_invoice(i_id):
    ...
    html = render_template('printouts/printable_template.html',
                           company=company.repr(),
                           object=invoice.repr(),
                           titre="Facture d'achat" if Order.query.get(invoice.fk_order_id).category=="achat" else 'Facture de vente',
                           total_letters=str.upper(total_letters))
...
from root.models import DeliveryNote
...
def deliveries():
    ...
    if _deliveries:
        ...
        for delivery in _deliveries:
            ...
            liste.append(_dict)
            indexe += 1
    return render_template("admin/deliveries.html", liste=liste)


from flask_login import logout_user
@admin_bp.get('/delivery/<int:bl_id>/print')
@login_required
