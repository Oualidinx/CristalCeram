...
from root.models import Supplier, Client, Order, Contact, Utilisation
...
class NewItemForm(FlaskForm):
    ...
    format = QuerySelectField('Format ',blank_text="Séléctionner le format...",
                              allow_blank =True,
                              query_factory=lambda : Format.query.filter_by(
    ...
    utilisation = QuerySelectField('Format ', blank_text="Séléctionner la famille...", allow_blank=True,
                              query_factory=lambda: Utilisation.query.filter_by(
...
class FamillyForm(FlaskForm):
    label = StringField("Titre de l'utilisation:", validators=[DataRequired('Champs obligatoire')], render_kw={'data-placeholder':'Titre de l\'utilisation'})
    submit = SubmitField('Ajouter')
...
class EntryField(Form):
    ...
    def validate_unit_price(self, unit_price):
        if unit_price.data and float(unit_price.data) <= 0:
            raise ValidationError('Valeur invalide')

class PurchaseField(EntryField):
    unit_price = DecimalField('Prix d\'achat', validators=[DataRequired('Champs obligatoire')])
    sale_price = DecimalField('Prix de vente', validators=[DataRequired('Champs obligatoire')])
    def validate_unit_price(self, unit_price):
        if unit_price.data and float(unit_price.data) <= 0:
            raise ValidationError('Valeur invalide')
    ...
    def validate_sale_price(self, sale_price):
        if sale_price.data and float(sale_price.data) < 0:
            raise ValidationError('Valeur invalide')
